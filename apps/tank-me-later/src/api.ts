import type { CharacterInput, VoteRecord, HistoryPoint, BestRun, WarbandDefinition } from '@tml/shared/types'
import type { RankCutoff } from './solo-queue'

// Shared client calls live in the workspace package; re-exported here so the
// leaderboard components can keep importing them from './api'.
...
export async function fetchHistory(char: CharacterInput, list?: string): Promise<HistoryPoint[]> {
...
export async function fetchWarbandHistory(warbandId: string): Promise<HistoryPoint[]> {
  const params = new URLSearchParams({ kind: 'warband', id: warbandId })
  const res = await fetch(`/api/history?${params}`)
  if (!res.ok) return []
  return res.json()
}
